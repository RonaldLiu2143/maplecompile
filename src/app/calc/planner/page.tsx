...
import { defaultScouterInput } from "@/lib/scouter/types";
...
export default function PlannerPage() {
  ...
  useEffect(() => {
    ...
    const job = storage.getJobType() || last?.input?.jobType || "warrior";
    const char = storage.getCharType() || last?.input?.charType || "adele";
    ...
    if (last?.input) {
      setScouter({
        ...
        ...last.input,
        jobType: last.input.jobType || job,
        charType: last.input.charType || char,
      });
    } else {
      setScouter(defaultScouterInput(job, char));
      setError(
        "No saved scouter loadout found — using defaults. Open Scouter and save stats for accurate FD.",
      );
    }
    setHydrated(true);
  }, []);
  ...
  const pieces = useMemo(
    () => (hydrated ? flattenEquips(setup, flames, overrides) : []),
    [hydrated, setup, flames, overrides],
  );

  const equipCount = pieces.length;
  const hasScouter = scouter != null;
  ...
  useEffect(() => {
    if (!hydrated || !scouter || equipCount === 0 || activeKinds.length === 0) {
      setRanked([]);
      return;
    }
    let cancelled = false;
    ...
    (async () => {
      try {
        // Lazy-load cubing rates + ranker (cubeRates.json is large).
    ...
    (slotKey: string, patch: Partial<PlannerOverrides[string]>) => {
      setOverrides((prev) => {
        ...
        const cur = prev[slotKey] ?? {
          starForce: piece?.starForce ?? defaultStarForce(piece?.equip.level ?? 140),
          potentialTier:
            piece?.potentialTier ??
    ...
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-2 max-w-3xl text-sm opacity-75">
          ...
          using your Equipment Setup + Scouter stats. Interactive server mode is
      ...
      <section className="rounded-xl border border-border/50 bg-surface/90 p-4 sm:p-5">
        ...
        {equipCount === 0 && (
          <p className="mt-4 text-sm opacity-80">
            No gear in Equipment Setup yet. Load a starter loadout or pick
            pieces, then come back — rankings need your current setup.
          </p>
        )}
      </section>
        ...
        <>
            ...
            <div className="mt-3 flex flex-wrap gap-4 text-sm">
              <label className="inline-flex items-center gap-2">
                ...
                <input
                  ...
                  value={rrfPrice}
                  onChange={(e) => setRrfPrice(Number(e.target.value) || 0)}
                />
              </label>
              <label className="inline-flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-28 rounded border border-border bg-background px-2 py-1 outline-none focus:border-accent"
                  value={crfPrice}
                  onChange={(e) => setCrfPrice(Number(e.target.value) || 0)}
                />
              </label>
            </div>
            <p className="mt-2 text-[11px] opacity-55">
              Efficiency = FD% ÷ (meso / 1B). SF costs use GMS wiki formulas + EV
              with safeguard 15–17. Flame/cube FD deltas are approximate.
            </p>
          </section>
          ...
          <section className="rounded-xl border border-border/50 bg-surface/90 p-4 sm:p-5">
            ...
            <p className="mt-1 text-xs opacity-60">
              Defaults assume mid-Heroic stars/tiers when unset. Edit to match
              your real gear before trusting ranks.
            </p>
                ...
                <tbody>
                  {pieces.map((p) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-1.5 tabular-nums">{p.equip.level}</td>
                      <td className="px-3 py-1.5">
                        <input
                          type="number"
                          min={0}
          ...
          <section className="rounded-xl border border-border/50 bg-surface/90 p-4 sm:p-5">
            ...
            {ranked.length === 0 ? (
              <p className="text-sm opacity-75">
                ...
                pot values, enable upgrade types, or fill scouter stats.
              </p>
            ) : (
